...
import {
  ...
  Trophy, Users, ArrowRight, ChevronLeft, Lock,
      ...
      <table className="w-full">
        <thead>
          <tr className="bg-slate-50 dark:bg-white/[0.02] border-b border-slate-200 dark:border-white/5">
            <th className="px-4 py-3 text-left text-[10px] font-black text-slate-400 dark:text-slate-600 uppercase tracking-widest">#</th>
            <th className="px-4 py-3 text-left text-[10px] font-black text-slate-400 dark:text-slate-600 uppercase tracking-widest">Peserta</th>
            <th className="px-4 py-3 text-center text-[10px] font-black text-slate-400 dark:text-slate-600 uppercase tracking-widest">TWK</th>
            <th className="px-4 py-3 text-center text-[10px] font-black text-slate-400 dark:text-slate-600 uppercase tracking-widest">TIU</th>
            <th className="px-4 py-3 text-center text-[10px] font-black text-slate-400 dark:text-slate-600 uppercase tracking-widest">TKP</th>
            <th className="px-4 py-3 text-center text-[10px] font-black text-slate-400 dark:text-slate-600 uppercase tracking-widest">Total</th>
          </tr>
        </thead>
          ...
          {results.map((r, i) => {
            ...
            return (
              <tr key={r.id} className={cn(
                "transition-colors",
                isMe ? "bg-indigo-50/60 dark:bg-indigo-500/5" : "hover:bg-slate-50 dark:hover:bg-white/[0.02]"
              )}>
                <td className="px-4 py-3">
                  {i === 0 ? <span className="text-yellow-500 font-black text-sm">🥇</span>
                    : i === 1 ? <span className="text-slate-400 font-black text-sm">🥈</span>
                    : i === 2 ? <span className="text-amber-600 font-black text-sm">🥉</span>
                    : <span className="text-slate-400 dark:text-slate-600 text-sm font-bold">{i + 1}</span>}
                </td>
                <td className="px-4 py-3">
                  <div className="flex items-center gap-2">
                    <div className="w-7 h-7 rounded-lg bg-indigo-100 dark:bg-indigo-500/10 border border-indigo-200 dark:border-indigo-500/20 flex items-center justify-center text-indigo-600 dark:text-indigo-400 font-black text-xs">
                      {name.charAt(0).toUpperCase()}
                    </div>
                    <span className={cn("text-sm font-bold truncate max-w-[120px]", isMe ? "text-indigo-600 dark:text-indigo-400" : "text-slate-900 dark:text-white")}>
                      {name}{isMe && " (Kamu)"}
                    </span>
                  </div>
                </td>
                ...
                <td className="px-4 py-3 text-center">
                  <span className={cn(
                    ...
                    i === 0 ? "bg-yellow-50 dark:bg-yellow-500/10 text-yellow-600 dark:text-yellow-400"
                      : isMe ? "bg-indigo-50 dark:bg-indigo-500/10 text-indigo-600 dark:text-indigo-400"
...
function EventCard({
  ...
  return (
    <div className="bg-white dark:bg-[#0d0d14] border border-slate-200 dark:border-white/5 rounded-2xl overflow-hidden flex flex-col transition-all duration-300 hover:border-indigo-200 dark:hover:border-indigo-500/20 hover:shadow-lg dark:hover:shadow-none">
      {/* Header */}
      <div className="p-5 border-b border-slate-100 dark:border-white/5">
        <div className="flex items-start justify-between gap-3 mb-3">
          <div className="flex items-center gap-2 flex-wrap">
            <span className={cn("px-2.5 py-1 rounded-lg text-[9px] font-black uppercase tracking-widest border", categoryColor[cat] || categoryColor.SKD)}>
              {cat}
            </span>
            {event.is_active && !ended ? (
              <span className="px-2.5 py-1 rounded-lg text-[9px] font-black uppercase tracking-widest border bg-emerald-500/10 text-emerald-400 border-emerald-500/20">
                Aktif
              </span>
            ) : (
              <span className="px-2.5 py-1 rounded-lg text-[9px] font-black uppercase tracking-widest border bg-slate-100 dark:bg-slate-500/10 text-slate-500 border-slate-200 dark:border-slate-500/20">
        ...
        <h3 className="text-base font-black text-slate-900 dark:text-white leading-tight mb-1">{event.title}</h3>
        {event.description && (
        ...
        {userResult ? (
          <div className="flex items-center gap-3 p-3 bg-emerald-50 dark:bg-emerald-500/5 border border-emerald-200 dark:border-emerald-500/20 rounded-xl">
            ...
            <div className="flex-1 min-w-0">
              <p className="text-xs font-black text-emerald-700 dark:text-emerald-400">Sudah Dikerjakan</p>
              <p className="text-[11px] text-emerald-600 dark:text-emerald-500 mt-0.5">Skor: <span className="font-black">{userResult.total}</span></p>
            </div>
          </div>
        ) : canPlay ? (
          <button
            onClick={() => onStart(event)}
            className="w-full flex items-center justify-center gap-2 h-10 bg-indigo-600 hover:bg-indigo-500 text-white rounded-xl font-black text-xs uppercase tracking-widest transition-all active:scale-95 shadow-lg shadow-indigo-500/20"
          >
      ...
      {showLeaderboard && (
        <div className="border-t border-slate-100 dark:border-white/5 p-5">
          <div className="flex items-center gap-2 mb-4">
            ...
            <h4 className="text-sm font-black text-slate-900 dark:text-white">Leaderboard Event</h4>
          </div>
    ...
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-2 duration-500">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <h1 className="text-2xl font-black text-slate-900 dark:text-white tracking-tight">Event &amp; Tryout Akbar</h1>
      ...
      {loading ? (
        <div className="flex flex-col items-center justify-center py-20 gap-4">
          ...
          <p className="text-slate-400 dark:text-slate-600 font-bold uppercase text-[10px] tracking-widest">Memuat Event...</p>
        </div>
      ) : events.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-20 px-6 text-center bg-white dark:bg-[#0d0d14] border border-dashed border-slate-200 dark:border-white/5 rounded-2xl">
          ...
          <h3 className="text-lg font-black text-slate-900 dark:text-white tracking-tight mb-2">Belum Ada Event</h3>
        ...
        <>
          ...
          {pastEvents.length > 0 && (
            <div className="space-y-4">
              <h2 className="text-sm font-black text-slate-500 dark:text-slate-600 uppercase tracking-widest flex items-center gap-2">
      ...
      <div className="bg-white dark:bg-[#0d0d14] border border-indigo-100 dark:border-indigo-500/20 rounded-2xl p-8 text-center relative overflow-hidden">
        ...
        <div className="relative z-10 space-y-4">
          ...
          <h2 className="text-xl font-black text-slate-900 dark:text-white tracking-tight">Ada Pertanyaan Mengenai Event?</h2>
          ...
          <a
            ...
            target="_blank"
            rel="noreferrer"
            className="inline-flex items-center gap-2 px-6 py-3 bg-indigo-600 hover:bg-indigo-500 text-white font-black text-xs uppercase tracking-widest rounded-xl transition-all shadow-lg shadow-indigo-500/20 active:scale-95"
          >
