    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-slate-500 text-sm mt-1">Pantau semua status pemesanan paket belajar Anda.</p>
        </div>
      </div>
      ...
      {transactions.length === 0 ? (
        ...
        <div className="grid gap-4">
          {transactions.map((tx) => (
            <div 
              key={tx.id}
              className="bg-white rounded-3xl border border-slate-100 shadow-sm hover:shadow-md transition-all p-5 flex flex-col sm:flex-row items-center gap-6"
            >
              ...
              <div className="flex-1 text-center sm:text-left">
                <div className="flex flex-wrap items-center justify-center sm:justify-start gap-2 mb-1">
                  <h3 className="font-black text-slate-900">{tx.packages?.title}</h3>
                ...
                <div className="flex flex-wrap items-center justify-center sm:justify-start gap-x-4 gap-y-1 text-xs text-slate-400 font-bold">
              ...
              <div className="flex flex-col items-center sm:items-end gap-3">
                ...
                {tx.status === 'pending' ? (
                  ...
                  <button 
                    onClick={() => onViewInvoice(tx.id)}
                    className="flex items-center gap-2 px-4 py-2 bg-slate-100 hover:bg-slate-200 text-slate-600 text-[10px] font-black uppercase tracking-widest rounded-xl transition-all"
                  >
